...
import { 
  ...
  Award, Activity, Sparkles, Clock, BarChart3, Copy, RotateCcw, Trash2, Heart
} from 'lucide-react';
...
export default function TeamDetailsPage() {
  ...
  const [suggestedQuestions, setSuggestedQuestions] = useState<string[]>([
    '🎯 What formation does this team typically play and why?',
    '⚡ Who are the key playmakers in this squad?',
    '🛡️ What is this team\'s main defensive weakness?',
    '📈 How has this team performed in recent World Cups?',
    '🤝 Compare this team\'s midfield to the top-ranked sides',
    '🔥 What pressing trigger does this team use to win the ball back?',
  ]);
  ...
  if (teamLoading) {
    return (
      <div className="flex h-screen w-screen items-center justify-center bg-white dark:bg-[#070e0a] text-zinc-600 dark:text-zinc-400">
        <div className="flex flex-col items-center gap-3">
          <Spinner size="lg" />
          <p className="text-xs font-semibold tracking-wider uppercase text-zinc-500">Loading squad details...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col min-h-screen bg-white dark:bg-[#070e0a]">
        ...
        <main className="flex-1 flex flex-col items-center justify-center p-12 text-center max-w-lg mx-auto gap-4">
          ...
          <h2 className="text-xl font-bold text-zinc-900 dark:text-white">Squad Not Found</h2>
          <p className="text-xs text-zinc-600 dark:text-zinc-400 font-medium">The team profile could not be loaded or database record does not exist.</p>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl bg-zinc-950/80 border border-emerald-950/80 shadow-sm hover:scale-105 transition-all cursor-pointer flex items-center gap-1.5 text-xs font-bold"
                ...
                >
                  ...
                  <span className={isFavorited(team._id) ? 'text-rose-400' : 'text-zinc-400'}>
                    {isFavorited(team._id) ? 'Favorited' : 'Favorite'}
                  </span>
                </button>
              </div>
                  ...
                  relatedTeams.map((t) => (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <span className="text-[9px] text-zinc-500 block">FIFA Rank: #{t.fifaRanking}</span>
                      </div>
                    </Link>
                  ))
                ) : (
                  <p className="text-[10px] text-zinc-500 italic">No related squads found in database.</p>
                )}
              </div>
            </div>
